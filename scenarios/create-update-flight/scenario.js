...
var xml_update = pd.xmlmin(raw).replace(/\$\{flno\}/g, flFields.flno).replace('${prefix}', prefix).replace('${departureDate}', newDepDate).replace('${arrivalDate}', flFields.arrivalDate)
    .replace(/\$\{departureAirport\}/g, flFields.departureAirport).replace('${arrivalAirport}', flFields.arrivalAirport).replace('${serviceType}', flFields.serviceType);
